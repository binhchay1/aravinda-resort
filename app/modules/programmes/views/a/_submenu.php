...
<?php if(IS_ROOT) : ?>
    <ul class="nav nav-tabs">
     ...
     <li <?= ($action === 'edit-lang' && SEG6 == $item[1]) ? 'class="active"' : '' ?>><a href="<?= Url::to(['/admin/'.$module.'/a/edit-lang/'.$model->primaryKey.'/'.$item[1]]) ?>">Edit <?=$item[2]?></a></li>
 <? endforeach; ?>
       
        <li <?= ($action === 'fields') ? 'class="active"' : '' ?>><a href="<?= Url::to(['/admin/'.$module.'/a/fields', 'id' => $model->primaryKey]) ?>"><span class="glyphicon glyphicon-cog"></span> <?= Yii::t('easyii/programmes', 'Fields') ?></a></li>
