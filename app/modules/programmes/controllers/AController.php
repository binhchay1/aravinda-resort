...
public function actionEditLang($id, $lang)
{
    ...
    if(!($modelLang = CategoryLang::findOne(['cat_id' => $id, 'lang_code' => $lang]))){
...
if ($modelLang->load(Yii::$app->request->post())) {
    if(Yii::$app->request->isAjax){
        ...
        $modelLang->cat_id = $id;
        $modelLang->content = $modelLang->content;
        if ($modelLang->save()) {
            $this->flash('success', 'Category updated');
            return $this->redirect(['/admin/'.$this->module->id.'/a/edit-lang/'.$id.'/'.$lang]);
        } else {
            $this->flash('error', Yii::t('easyii', 'Update error. {0}', $model->formatErrors()));
            return $this->refresh();
        }
