...
use app\modules\tours\assets\FieldsAsset;
use app\modules\tours\models\Category;

$this->title = Yii::t('easyii/tours', 'Category fields');
...
var fieldTemplate = \'\
    <tr>\
        ...
        <td><textarea class="form-control field-options" placeholder="'.Yii::t('easyii/tours', 'Type options with `comma` as delimiter').'" style="display: none;"></textarea></td>\
        <td class="text-right">\
...
<?= Html::button('<i class="glyphicon glyphicon-plus font-12"></i> '.Yii::t('easyii/tours', 'Add field'), ['class' => 'btn btn-default', 'id' => 'addField']) ?>

<table id="categoryFields" class="table table-hover">
    <thead>
        <th>Name</th>
        <th><?= Yii::t('easyii', 'Title') ?></th>
        <th><?= Yii::t('easyii/tours', 'Type') ?></th>
        <th><?= Yii::t('easyii/tours', 'Options') ?></th>
        <th width="120"></th>
    </thead>
    <tbody>
    <?php foreach($model->fields as $field) : ?>
        <tr>
            ...
            <td>
                <textarea class="form-control field-options" placeholder="<?= Yii::t('easyii/tours', 'Type options with `comma` as delimiter') ?>" <?= !$field->options ? 'style="display: none;"' : '' ?> ><?= is_array($field->options) ? implode(',', $field->options) : '' ?></textarea>
            </td>
            <td class="text-right">
                <div class="btn-group btn-group-sm" role="group">
                    <a href="#" class="btn btn-default move-up" title="<?= Yii::t('easyii', 'Move up') ?>"><span class="glyphicon glyphicon-arrow-up"></span></a>
...
<?= Html::button('<i class="glyphicon glyphicon-ok"></i> '.Yii::t('easyii/tours', 'Save fields'), ['class' => 'btn btn-primary', 'id' => 'saveCategoryBtn']) ?>
