    ...
    <div class="container">
        ...
        <div class="row text-cente no-gutters amthuc-row-3">
            ...
            <? foreach($theEntry->items() as $k => $v) : ?>
                ...
                <?
                $vimg = ''; 
                if(isset($v->photosArray['summary'][0])) $vimg = $v->photosArray['summary'][0]; ?>
                <div class="mb-130 pb-0 text-left villa-item col-12 col-sm-12 col-lg-4 text-left">
                <img alt="<?=$vimg ? $vimg->description : ''?>" src="/timthumb.php?src=<?=$vimg ? $vimg->image :'' ?>&w=560&zc=0&q=80" class="w-100">
                <h2 class="tt my-40 text-capitalize"><?=$v->title; ?></h2></a>
